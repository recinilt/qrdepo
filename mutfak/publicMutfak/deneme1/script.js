...
// Örnek: "018699651521297":{name:"TROPISAL 0.5 MG+2,5 MG/2,5 ML NEBULIZASYON ICIN INHALASYON COZELTISI (20 FLAKON)",unitCount:1}
let productNames = { 
  "0108680760092235": { name: "Ibucold C 30 tb", unitCount: 30 } 
}; 
//ilacgtinleri.js dosyasından bu değişken atanıyor.
    ...
    function loadProductNames() {
      ...
      if (storedNames) {
        productNames = JSON.parse(storedNames);
      } else {
    ...
    function startScanning() {
      ...
      qrCodeScanner.start(
        ...
        (errorMessage) => {
          console.log(`Tarama başarısız: ${errorMessage}`);
        }
      ).catch(err => {
    ...
    function processScannedCode(decodedText) {
      // QR kodunu boşluklara göre ayırmayı dene
      ...
      if (parts.length >= 3) {
        ...
        const product = productNames[productCode] || { name: "Bilinmeyen Ürün", unitCount: 1 }; // Varsayılan olarak birim sayısı 1
        ...
        addProductToTable(decodedText, "N/A", "N/A", "Barkod", decodedText, 1);
      }
    }
    ...
    function updateSummaryTable() {
      ...
      rows.forEach(row => {
        ...
        const unitCount = parseInt(row.cells[0].querySelector('.unit-count').value) || 1; // Birim sayısını al
    ...
    function updateCurrentShelf() {
      ...
      virtualTableData.forEach(item => {
        const { productName, year, month, unitCount } = item;
        const yearMonth = `${year}-${month}`;
    ...
    function addProductToTable(productCode, year, month, productName, fullCode, unitCount = 1) {
      ...
      const existingRow = Array.from(productTableBody.rows).find(row => row.cells[5].textContent === fullCode);
      ...
      saveButton.addEventListener('click', () => {
        ...
        if (newProductName) {
          ...
          virtualTableData = virtualTableData.map(item => 
              item.timestamp === timestamp // Zaman damgasına göre eşleştir
      ...
      newRow.innerHTML = `
          ...
          <td>${fullCode}</td>
      `;
      newRow.insertBefore(actionsCell, newRow.firstChild); // "Ürün Adı - İçerik Sayısı" hücresini başa ekle
      newRow.appendChild(deleteCell);

      productTableBody.appendChild(newRow);
      updateSummaryTable();

      // Sanal tabloya veri ekle (zaman damgasını da ekle)
  ...
  function deleteProductByTimestamp(timestamp) {
    ...
    rows.forEach(row => {
        ...
        if (rowTimestamp == timestamp) {
            productTableBody.deleteRow(row.rowIndex);
        }
    });
